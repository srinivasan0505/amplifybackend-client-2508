...
func ClonebackendHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CloneBackendrequest
		...
		url := fmt.Sprintf("%s/backend/%s/environments/%s/clone", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.CloneBackendResponse
...
func CreateClonebackendTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_environments_backendEnvironmentName_clone",
		mcp.WithDescription("This operation clones an existing backend."),
		...
		mcp.WithString("targetEnvironmentName", mcp.Required(), mcp.Description("Input parameter: The name of the destination backend environment to be created.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ClonebackendHandler(cfg),
	}
}
