...
func CreatetokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/backend/%s/challenge", cfg.BaseURL, appId)
...
func CreateCreatetokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_challenge",
		mcp.WithDescription("Generates a one-time challenge code to authenticate a user into your Amplify Admin UI."),
