...
func ImportbackendstorageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ImportBackendStoragerequest
		...
		url := fmt.Sprintf("%s/backend/%s/storage/%s/import", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.ImportBackendStorageResponse
...
func CreateImportbackendstorageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_storage_backendEnvironmentName_import",
		mcp.WithDescription("Imports an existing backend storage resource."),
		...
		mcp.WithString("bucketName", mcp.Description("Input parameter: The name of the S3 bucket.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ImportbackendstorageHandler(cfg),
	}
}
