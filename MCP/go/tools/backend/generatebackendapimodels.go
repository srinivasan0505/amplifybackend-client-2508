...
func GeneratebackendapimodelsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeleteBackendAuthrequest
		...
		url := fmt.Sprintf("%s/backend/%s/api/%s/generateModels", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.GenerateBackendAPIModelsResponse
...
func CreateGeneratebackendapimodelsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_api_backendEnvironmentName_generateModels",
		mcp.WithDescription("Generates a model schema for an existing backend API resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GeneratebackendapimodelsHandler(cfg),
	}
}
