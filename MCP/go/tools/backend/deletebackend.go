...
func DeletebackendHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/backend/%s/environments/%s/remove", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.DeleteBackendResponse
...
func CreateDeletebackendTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_environments_backendEnvironmentName_remove",
		mcp.WithDescription("Removes an existing environment from your Amplify project."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletebackendHandler(cfg),
	}
}
