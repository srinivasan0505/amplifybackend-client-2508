...
func DeletetokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/backend/%s/challenge/%s/remove", cfg.BaseURL, appId, sessionId)
		...
		var result models.DeleteTokenResponse
...
func CreateDeletetokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_challenge_sessionId_remove",
		mcp.WithDescription("Deletes the challenge token based on the given appId and sessionId."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletetokenHandler(cfg),
	}
}
