...
func GettokenHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/backend/%s/challenge/%s", cfg.BaseURL, appId, sessionId)
...
func CreateGettokenTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_backend_appId_challenge_sessionId",
		mcp.WithDescription("Gets the challenge token based on the given appId and sessionId."),
