...
func UpdatebackendauthHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateBackendAuthrequest
		...
		url := fmt.Sprintf("%s/backend/%s/auth/%s", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.UpdateBackendAuthResponse
...
func CreateUpdatebackendauthTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_auth_backendEnvironmentName",
		mcp.WithDescription("Updates an existing backend authentication resource."),
		...
		mcp.WithObject("resourceConfig", mcp.Required(), mcp.Description("Input parameter: Defines the resource configuration when updating an authentication resource in your Amplify project.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatebackendauthHandler(cfg),
	}
}
