...
func ImportbackendauthHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ImportBackendAuthrequest
		...
		url := fmt.Sprintf("%s/backend/%s/auth/%s/import", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.ImportBackendAuthResponse
...
func CreateImportbackendauthTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_auth_backendEnvironmentName_import",
		mcp.WithDescription("Imports an existing backend authentication resource."),
		...
		mcp.WithString("identityPoolId", mcp.Description("Input parameter: The ID of the Amazon Cognito identity pool.")),
		mcp.WithString("nativeClientId", mcp.Required(), mcp.Description("Input parameter: The ID of the Amazon Cognito native client.")),
		mcp.WithString("userPoolId", mcp.Required(), mcp.Description("Input parameter: The ID of the Amazon Cognito user pool.")),
		mcp.WithString("webClientId", mcp.Required(), mcp.Description("Input parameter: The ID of the Amazon Cognito web client.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ImportbackendauthHandler(cfg),
	}
}
