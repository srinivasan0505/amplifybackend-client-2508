...
func ListbackendjobsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListBackendJobsrequest
		...
		url := fmt.Sprintf("%s/backend/%s/job/%s%s", cfg.BaseURL, appId, backendEnvironmentName, queryString)
		...
		var result models.ListBackendJobsResponse
...
func CreateListbackendjobsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_job_backendEnvironmentName",
		mcp.WithDescription("Lists the jobs for the backend of an Amplify app."),
		...
		mcp.WithString("jobId", mcp.Description("Input parameter: The ID for the job.")),
		mcp.WithNumber("maxResults", mcp.Description("Input parameter: The maximum number of results that you want in the response.")),
		mcp.WithString("nextToken", mcp.Description("Input parameter: The token for the next set of results.")),
		mcp.WithString("operation", mcp.Description("Input parameter: Filters the list of response objects to include only those with the specified operation name.")),
		mcp.WithString("status", mcp.Description("Input parameter: Filters the list of response objects to include only those with the specified status.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListbackendjobsHandler(cfg),
	}
}
