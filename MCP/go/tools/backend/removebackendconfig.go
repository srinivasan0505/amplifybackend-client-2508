...
func RemovebackendconfigHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/backend/%s/config/remove", cfg.BaseURL, appId)
		...
		var result models.RemoveBackendConfigResponse
...
func CreateRemovebackendconfigTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_config_remove",
		mcp.WithDescription("Removes the AWS resources required to access the Amplify Admin UI."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    RemovebackendconfigHandler(cfg),
	}
}
