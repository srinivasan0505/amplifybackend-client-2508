...
func CreatebackendauthHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateBackendAuthrequest
		...
		url := fmt.Sprintf("%s/backend/%s/auth", cfg.BaseURL, appId)
		...
		var result models.CreateBackendAuthResponse
...
func CreateCreatebackendauthTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_auth",
		mcp.WithDescription("Creates a new backend authentication resource."),
		...
		mcp.WithObject("resourceConfig", mcp.Required(), mcp.Description("Input parameter: Defines the resource configuration when creating an auth resource in your Amplify project.")),
		mcp.WithString("resourceName", mcp.Required(), mcp.Description("Input parameter: The name of this resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatebackendauthHandler(cfg),
	}
}
