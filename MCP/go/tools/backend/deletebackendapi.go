...
func DeletebackendapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeleteBackendAPIrequest
		...
		url := fmt.Sprintf("%s/backend/%s/api/%s/remove", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.DeleteBackendAPIResponse
...
func CreateDeletebackendapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_api_backendEnvironmentName_remove",
		mcp.WithDescription("Deletes an existing backend API resource."),
		...
		mcp.WithObject("resourceConfig", mcp.Description("Input parameter: The resource config for the data model, configured as a part of the Amplify project.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeletebackendapiHandler(cfg),
	}
}
