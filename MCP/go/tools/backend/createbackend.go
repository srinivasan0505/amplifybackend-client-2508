...
func CreatebackendHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateBackendrequest
		...
		url := fmt.Sprintf("%s/backend", cfg.BaseURL)
		...
		var result models.CreateBackendResponse
...
func CreateCreatebackendTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend",
		mcp.WithDescription("This operation creates a backend for an Amplify app. Backends are automatically created at the time of app creation."),
		...
		mcp.WithString("resourceName", mcp.Description("Input parameter: The name of the resource.")),
		mcp.WithString("appId", mcp.Required(), mcp.Description("Input parameter: The app ID.")),
		mcp.WithString("appName", mcp.Required(), mcp.Description("Input parameter: The name of the app.")),
		mcp.WithString("backendEnvironmentName", mcp.Required(), mcp.Description("Input parameter: The name of the backend environment.")),
		mcp.WithObject("resourceConfig", mcp.Description("Input parameter: Defines the resource configuration for the data model in your Amplify project.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatebackendHandler(cfg),
	}
}
