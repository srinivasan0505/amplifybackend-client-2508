...
func CreatebackendconfigHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateBackendConfigrequest
		...
		url := fmt.Sprintf("%s/backend/%s/config", cfg.BaseURL, appId)
		...
		var result models.CreateBackendConfigResponse
...
func CreateCreatebackendconfigTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_config",
		mcp.WithDescription("Creates a config object for a backend."),
		...
		mcp.WithString("backendManagerAppId", mcp.Description("Input parameter: The app ID for the backend manager.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatebackendconfigHandler(cfg),
	}
}
