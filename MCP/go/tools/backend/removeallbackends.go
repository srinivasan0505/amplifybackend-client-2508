...
func RemoveallbackendsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.RemoveAllBackendsrequest
		...
		url := fmt.Sprintf("%s/backend/%s/remove", cfg.BaseURL, appId)
		...
		var result models.RemoveAllBackendsResponse
...
func CreateRemoveallbackendsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_remove",
		mcp.WithDescription("Removes all backend environments from your Amplify project."),
		...
		mcp.WithBoolean("cleanAmplifyApp", mcp.Description("Input parameter: Cleans up the Amplify Console app if this value is set to true.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    RemoveallbackendsHandler(cfg),
	}
}
