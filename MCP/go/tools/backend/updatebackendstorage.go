...
func UpdatebackendstorageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UpdateBackendStoragerequest
		...
		url := fmt.Sprintf("%s/backend/%s/storage/%s", cfg.BaseURL, appId, backendEnvironmentName)
		...
		var result models.UpdateBackendStorageResponse
...
func CreateUpdatebackendstorageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_storage_backendEnvironmentName",
		mcp.WithDescription("Updates an existing backend storage resource."),
		...
		mcp.WithObject("resourceConfig", mcp.Required(), mcp.Description("Input parameter: The resource configuration for updating backend storage.")),
		mcp.WithString("resourceName", mcp.Required(), mcp.Description("Input parameter: The name of the storage resource.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UpdatebackendstorageHandler(cfg),
	}
}
