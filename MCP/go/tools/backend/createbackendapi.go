...
func CreatebackendapiHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.CreateBackendAPIrequest
		...
		url := fmt.Sprintf("%s/backend/%s/api", cfg.BaseURL, appId)
		...
		var result models.CreateBackendAPIResponse
...
func CreateCreatebackendapiTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_backend_appId_api",
		mcp.WithDescription("Creates a new backend API resource."),
		...
		mcp.WithObject("resourceConfig", mcp.Required(), mcp.Description("Input parameter: The resource config for the data model, configured as a part of the Amplify project.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CreatebackendapiHandler(cfg),
	}
}
