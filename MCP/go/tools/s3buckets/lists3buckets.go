...
func Lists3bucketsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListS3Bucketsrequest
		...
		url := fmt.Sprintf("%s/s3Buckets", cfg.BaseURL)
		...
		var result models.ListS3BucketsResponse
...
func CreateLists3bucketsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_s3Buckets",
		mcp.WithDescription("The list of S3 buckets in your account."),
		...
		mcp.WithString("nextToken", mcp.Description("Input parameter: Reserved for future use.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Lists3bucketsHandler(cfg),
	}
}
