/**
 * MCP Server function for Lists the jobs for the backend of an Amplify app.
 */
...
class Post_Backend_App_Id_Job_Backend_Environment_NameMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Backend_App_Id_Job_Backend_Environment_NameHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("operation")) {
            queryParams.add("operation=" + args.get("operation"));
        }
        ...
        if (args.containsKey("maxResults")) {
            queryParams.add("maxResults=" + args.get("maxResults"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_backend_app_id_job_backend_environment_name" + queryString;
    ...
    public static MCPServer.Tool createPost_Backend_App_Id_Job_Backend_Environment_NameTool(MCPServer.APIConfig config) {
        ...
        jobIdProperty.put("description", "Input parameter: The ID for the job.");
        properties.put("jobId", jobIdProperty);
        Map<String, Object> nextTokenProperty = new HashMap<>();
        nextTokenProperty.put("type", "string");
        nextTokenProperty.put("required", false);
        nextTokenProperty.put("description", "Input parameter: The token for the next set of results.");
        properties.put("nextToken", nextTokenProperty);
        Map<String, Object> operationProperty = new HashMap<>();
        operationProperty.put("type", "string");
        operationProperty.put("required", false);
        operationProperty.put("description", "Input parameter: Filters the list of response objects to include only those with the specified operation name.");
        ...
        statusProperty.put("description", "Input parameter: Filters the list of response objects to include only those with the specified status.");
        properties.put("status", statusProperty);
        Map<String, Object> maxResultsProperty = new HashMap<>();
        maxResultsProperty.put("type", "string");
        maxResultsProperty.put("required", false);
        maxResultsProperty.put("description", "Input parameter: The maximum number of results that you want in the response.");
        properties.put("maxResults", maxResultsProperty);
        parameters.put("properties", properties);
        
        MCPServer.ToolDefinition definition = new MCPServer.ToolDefinition(
            "post_backend_app_id_job_backend_environment_name",
            "Lists the jobs for the backend of an Amplify app.",
            parameters
        );
        
        return new MCPServer.Tool(definition, getPost_Backend_App_Id_Job_Backend_Environment_NameHandler(config));
    }
